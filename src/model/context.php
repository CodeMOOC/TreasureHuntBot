...
class Context {
    ...
    private $game_has_timeout = false;
    ...
    /**
     * Returns whether the game has a timeout.
     */
    function has_timeout() {
        return $this->game_has_timeout;
    }
    ...
    function is_timed_out() {
        return ($this->game_has_timeout && $this->game_timed_out);
    }
    ...
    function refresh() {
        ...
        if($game !== null) {
            ...
            $group = db_row_query("SELECT `groups`.`game_id`, `groups`.`name`, `groups`.`state`, `games`.`event_id`, `games`.`state`, `games`.`telegram_channel`, `games`.`telegram_channel_censor_photo`, (`games`.`timeout_absolute` IS NOT NULL OR `games`.`timeout_interval` IS NOT NULL) AS `has_timeout`, IF(`groups`.`timeout_absolute` IS NULL, 0, TIMEDIFF(NOW(), `groups`.`timeout_absolute`) > 0) AS `timed_out` FROM `groups` LEFT OUTER JOIN `games` ON `groups`.`game_id` = `games`.`game_id` WHERE `group_id` = {$this->get_user_id()} ORDER BY `groups`.`registered_on` DESC LIMIT 1");
            if($group !== null) {
                ...
                $this->game_has_timeout = (bool)$group[7];
                $this->game_timed_out = (bool)$group[8];
